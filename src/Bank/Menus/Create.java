...
public class Create {
    public Client createClient(){
        ...
        int valid = connect.execute("INSERT INTO \"user\" (name, last_name, user_name, password) VALUES('" + name + "', '" + lastName +"', '" + userName + "', '" + password +"')");
        if(valid == 1){
    ...
    public static void createAccount(User user) throws SQLException {
        ...
        int valid = connect.execute("INSERT INTO account (currency, balance, type, user_id) VALUES ('" + currency + "', 0, '" + type +"', " + id + ")");
        if(valid == 1){
    ...
    public static void createTransaction(User  loggedUser){
        ...
        if(destinyAccount != null){
            ...
            System.out.println("------------------------");
            if(!originAccount.getCurrency().equals(destinyAccount.getCurrency())){
                System.out.println(originAccount.getCurrency() + " " + destinyAccount.getCurrency());
                System.out.println("La divisa de la cuenta seleccionada no corresponde a la divisa de la cuenta origen");
                return;
            }
            ...
            if(amount > originAccount.getBalance()){
                ...
                switch (option){
                    case 1:
                        int valid = connect.execute("INSERT INTO \"transaction\" (date, amount, origin_account_id, destiny_account_id) VALUES (CURRENT_DATE, " + amount + "," + originAccount.getAccountID() +", " + destinyAccount.getAccountID() + ")");
                        if(valid == 1){
